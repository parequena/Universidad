...
public class AnalizadorLexico
{
    ...
    isFinal = { false, true, true, true, true, true, true, true, true, true, false, true, false, true, false, false,
    ...
    private char read()
    {
        ...
        else // != eof()
        {
            ...
            else ret = (char) Token.EOF; // Static constant of the class.

            // if (aRow < file.size())
            // System.out.println("ret="+ret+", ret(int)="+(int)ret+", aRow="+aRow+",
            // aCol="+aCol+", linea=("+file.get(aRow)+")");
    ...
    // Prints an error.
    private void LexicalError(char c, Boolean comment)
    {
        if (c == Token.EndOfFile)
        {
            if (comment)
                System.err.println("Error lexico: fin de fichero inesperado");
            else return;
        }
        else System.err.println("Error lexico (" + row + "," + column + "): caracter ’" + c + "’ incorrecto");
    ...
    private int stateToType(int state)
    {
        ...
        if (state == 11)
            ...
            return 13; // 13 entero* // 17 entero**
        else if (state == 16)
            return 14; // 16 real*
        else return state - 1; // 1 pari (0) // 2 pard (1) // 3 coma (2) // 4 amp (3) // 5 llavei (4) // 6
                               // llaved (5) // 7 asig (6) // 8 pyc (7) // 9 opas (8)
    }
    ...
    private int delta(int estado, char c)
    {
        ...
        case 0:
            if (c == '(')
                ...
                return 10; // If 'c' is on letters.
            else if (numbers.indexOf(c) != -1)
                return 12; // If 'c' is on numbers.
            else if (c == '/')
                return 18; // If we got '/'
            else return -1;

        case 10:
            if (letters.indexOf(c) != -1 || numbers.indexOf(c) != -1)
                return 10;
            else return 11;
        ...
        case 19:
            if (c == Token.EndOfFile) LexicalError(Token.EndOfFile, true); // Fin de fichero inesperado.
            if (c == '*')
                return 20; // Iterate until reach '*'
            else return 19;

        case 20:
            if (c == '/')
                return 0; // If we found '*/'
            else return 19;
    ...
    public Token siguienteToken()
    {
        ...
        if (buffer.charAt(0) == Token.EndOfFile) return new Token(oriRow, oriCol, buffer, Token.EOF); // Creating the
                                                                                                      // EOF token.
        ...
        do
        {
            ...
            if (newState == -1)
            {
                LexicalError(buffer.charAt(buffer.length() - 1), false);
                return new Token(oriRow, oriCol, buffer, Token.EOF);
            }
            ...
            {
                if (newState == 0) System.out.print("");
                state = newState;
                buffer += read();
                if (buffer.charAt(0) == Token.EndOfFile) return new Token(oriRow, oriCol, buffer, Token.EOF); // Creating
                                                                                                              // the EOF
                                                                                                              // token.
                while (Character.isWhitespace(buffer.charAt(0))) // if buffer[0] == ' '
                {
